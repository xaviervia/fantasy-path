...
Path
  ...
  .chain(([sourcePath, targetPath]) => targetPath
    .strictDiffWith(sourcePath)
    .map(diffPath => [sourcePath, targetPath, diffPath])
  )
  .chain(([sourcePath, targetPath, diffPath]) =>
    sourcePath
      .strictAdd(diffPath)
      .map(updatedSourcePath => [
        updatedSourcePath,
        targetPath,
        diffPath
      ])
  )
  .map(([sourcePath, targetPath, diffPath]) => {
    render(
      <main
        style={{
          width: '100vw',
